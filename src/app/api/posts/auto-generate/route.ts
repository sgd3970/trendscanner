...
export async function POST(request: Request) {
  try {
    ...
    for (const { keyword, _id } of selectedKeywords) {
      try {
        const prompt = `
          당신은 블로그 트렌드 칼럼니스트입니다. 아래 키워드를 기반으로 블로그 글을 작성하세요.

          🧾 주의 사항:
          - 전체 글자 수: 최소 **1500자 이상**
          - **정보 중심의 블로그 스타일**, 인사이트를 담아주세요
          - 소제목은 **문장을 기반으로**, 지루하지 않고 호기심을 유발하도록 구성
          - 소제목은 최소 3개 이상, **예시: "왜 요즘 사람들은 OOO에 주목할까?"**
          - 텍스트에는 통계, 사례, 인용 등의 구체적인 정보도 포함해주세요

          📦 출력 형식 (JSON only):
          {
            "title": "자연스럽고 흥미로운 블로그 제목",
            "content": "## (소제목1)\\n\\n내용...\\n\\n## (소제목2)\\n\\n내용...\\n\\n## (소제목3)\\n\\n내용...",
