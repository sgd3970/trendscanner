  ...
  try {
    ...
    for (const { keyword, _id } of selectedKeywords) {
      try {
        ...
        const prompt = `
        당신은 SEO에 최적화된 블로그 작가 AI입니다. 아래 키워드를 바탕으로 **가독성이 뛰어난 블로그 포스트**를 작성해주세요. **한국어로만** 작성하며, 아래 구조를 **정확하게 JSON 형식**으로 따르세요:
        
        {
          "title": "트렌디한 블로그 제목 (한국어)",
          "content": "# 주제 개요\\n\\n블로그 도입부 설명 (300자 이상)\\n\\n## 트렌드 분석\\n\\n최근 동향에 대한 설명과 관련 사례 (400자 이상)\\n\\n## 전문가 시각\\n\\n전문가 의견 또는 통찰 제공 (300자 이상)\\n\\n## 결론 및 전망\\n\\n요약 및 향후 예상 (300자 이상)",
          "hashtags": ["키워드1", "키워드2", "키워드3", "키워드4", "키워드5"],
          "imageQuery": "영어로 된 이미지 검색 키워드"
        }
