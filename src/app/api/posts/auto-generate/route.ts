...
export async function POST(request: Request) {
  try {
    ...
    for (const { keyword, _id } of selectedKeywords) {
      try {
        const prompt = `
          당신은 최신 이슈를 흥미롭게 풀어내는 트렌드 블로그 작가입니다. 아래 키워드를 바탕으로 **독자 친화적인 블로그 글**을 작성해 주세요.
          
          작성 지침:
          - 전체 글자 수는 **1000자 이상**이어야 합니다.
          - 반드시 **한국어로 작성**해 주세요.
          - 문단마다 자연스럽고 현실적인 소제목을 붙여 주세요. (예: "왜 이 이슈가 화제일까?", "전문가들도 놀란 변화", "사람들은 이렇게 반응했다" 등)
          - 소제목은 **3개 이상**, 글의 흐름에 맞춰 자유롭게 구성하세요.
          - 말투는 **친근하고 설명하듯이** 작성해 주세요.
          
          응답은 아래 JSON 형식으로만 주세요. JSON 외 텍스트는 절대 포함하지 마세요.
